...
local GUI = {} -- ne pas toucher
ESX = nil -- ne pas toucher
GUI.Time = 0 -- ne pas toucher
local PlayerData = {} -- ne pas toucher
local showPro = false -- ne pas toucher
local stealing = false -- ne pas toucher
local peeking = false -- ne pas toucher
...
local chancePoliceNoti = 0 -- the procent police get notified (only numbers like 30, 10, 40. You get it.)
local useBlip = true -- si tu veux afficher le point sur la map
local useInteractSound = true -- if you wanna use InteractSound (when u lockpick the door)
------------------------------------------------------
------------------------------------------------------

------ l o c a l e s ------
local noCar = "Aucun véhicule à proximité"
local text = "Depuis votre inventaire prenez le ~r~Crochet~w~ pour crocheter la porte." -- lockpick the door
local textUnlock = "~g~[E]~w~ Entrer" -- entrer dans la maison
local insideText = "~g~[E]~w~ Sortir" -- quitter la maison
local abortConfirm = "Vous avez annulé la transaction"
local searchText = "~g~[E]~w~ Chercher par ici" -- fouille le spot
local emptyMessage = "Rien ici!" -- si vous chercher est qu'il y a rien 
local emptyMessage3D = "~r~vide" -- si le spot est vide
local closetText = "~g~[E]~w~ Cachez-vous dans l'armoire" -- text at closet
local abortLock = "Appuyez sur la touche ~g~[E]~w~ pour annuler l'opération."
local noLockpickText = "Vous n'avez pas de crochet!" -- si vous n'avez pas de crochet et que vous essayez de faire le cambriolage
local carUnlocked = "Vous avez réussi à déverrouiller la voiture!"
local youFound = "" -- quand tu voles quelque chose
local burglaryDetected = "Quelqu'un essaie de voler une maison!" -- texte police message 1
local sentPhoto = "Nous vous avons envoyé la photo du criminel." -- si vous utilisez le système de caméras de qalle, ce sera également dans le message
    ...
    if DoesEntityExist(vehicle) then
      ...
      TriggerServerEvent('99kr-burglary:removeKit')
        ...
        if CurrentAction ~= nil then
          ...
          { 
            type = 'inform', 
            text = 'Vous avez réussi à déverrouiller le véhicule!', 
            length = 4000 
          })
				end
		...
		Citizen.CreateThread(function()
          ...
          { 
            type = 'inform', 
            text = 'Vous avez annulé le processus.', 
            length = 4000 
          })
					CurrentAction = nil
				end
...
residents = {
	{
		...
		aggressive = true -- s'ils doivent attaquer après le réveil
	}
}
...
Citizen.CreateThread(function()
  while true do
    ...
    if GetClockHours() > 23  or GetClockHours() <= 22 then
        ...
        if dist <= 1.2 and timer == true then
         local secsRemaining = math.ceil((v.doorTime - GetGameTimer()) / 1000)
          ...
          if secsRemaining > 0 then
                DrawText3D(v.pos.x, v.pos.y, v.pos.z,'Veuillez attendre ~r~'..secsRemaining..'~p~ secondes ~w~ avant de pouvoir commencer !', 0.4)
          else
            timer = false
            v.locked = true 
            doorTime = {}      
      ...
      DrawText3D(v.pos.x, v.pos.y, v.pos.z, 'Veuillez entrer à nouveau ~r~' ..breakTime.. '~w~ attendre une heure', 0.4) 
      end
    end
    end
  end
          ...
          ESX.TriggerServerCallback('fizzfau_burglary:connectedcops', function(anycops)
            if anycops >= poli then
              if LockpickAmount > 0 then
                ...
                { 
                  type = 'inform', 
                  text = 'Vous n avez pas de crochet!', 
                  length = 4000 
                })
              end
            else
              exports['mythic_notify']:SendAlert('error', 'Pour ce faire, au moins ' .. poli .. ' dois être un flic!')
            end
          end)
	end)
end
...
if useBlip then
  Citizen.CreateThread(function()
    for k, v in pairs(burglaryPlaces) do
       ...
       AddTextComponentString('Maison civile')
...
local blips = {
  {title="Acheteur", colour=4, id=440, x = 244.61, y = 374.61, z = 105.74-0.98}
}
...
Citizen.CreateThread(function()
  while true do
      ...
      for k in pairs(gym) do
        ...
        if dist <= 0.5 then
          ESX.ShowHelpNotification('~INPUT_CONTEXT~ ~o~ Parlez au vendeur !')
          --DrawText3D(plyCoords.x, plyCoords.y, plyCoords.z, "~w~Esya satmak için ~b~[E] ~w~ Appuie sur le bouton!", 0.4)
...
function OpenSellMenu()
  ...
  ESX.UI.Menu.Open(
      ...
      {
        title    = 'Voici ce que nous achètons',
        elements = {
            {label = 'Bague ($750)', value = 'ring'},
            {label = 'Rolex ($750)', value = 'rolex'},
            {label = 'Caméra ($750)', value = 'camera'},
            {label = 'Collier en or ($1000)', value = 'goldNecklace'},
            {label = 'Ordinateur portable ($1000)', value = 'laptop'},
            {label = 'Téléphone ($350)', value = 'samsungS10'},
          }
      },
      function(data, menu)
